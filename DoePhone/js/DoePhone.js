...
function muteButton() {
	...
	if ( incall ) {
		if ( muted ) {
			...
			uiElements.mute_icon.src = 'images/microphone.png';
		} else {
			// call is not muted
			// mute it
			muted = true;
			debug_out( 'Mute Button Pressed' );
			uiElements.mute_icon.src = 'images/microphone-off.png';
		}
...
function setCallButtonStatus( status ) {
	if ( status ) {
		...
		uiElements.dial_icon.src = 'images/phone-off.png';
	}
	else {
		uiElements.dial.setAttribute('class', 'button dial');
		uiElements.dial_icon.src = 'images/phone-dial.png';
	}
}
...
function handleFailed( response, cause ) {
	...
	if ( cause == 'Canceled' ) {
		...
		if ( ua.isRegistered() ) {
			...
			uiElements.reg_icon.src = 'images/wp_register_active.png';
			uiElements.unreg_icon.src = 'images/wp_unregister_inactive.png';
		} else {
			uiElements.reg_status.value = 'Unregistered';
			uiElements.reg_icon.src = 'images/wp_register_inactive.png';
			uiElements.unreg_icon.src = 'images/wp_unregister_active.png';
		}
		my_session = false;
		return;
	}
